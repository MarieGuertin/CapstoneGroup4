...
void print_dfsdm_data() {
	...
	// buffer
	int32_t *read_buffer = (int32_t *) calloc(DEFAULT_BUFFER_LENGTH, DFSDM_DATA_WIDTH);

	// navigate through all DFSDM audio memory on flash
	while (printed_size < DFSDM_AUDIO_SIZE) {
		uint32_t remaining_size = DFSDM_AUDIO_SIZE - printed_size;
		uint32_t size = remaining_size < DEFAULT_BUFFER_LENGTH * DFSDM_DATA_WIDTH ? remaining_size : DEFAULT_BUFFER_LENGTH * DFSDM_DATA_WIDTH;

		// read from flash
		qspi_read((uint8_t*)read_buffer, DFSDM_AUDIO_QSPI_ADDRESS + printed_size, size);

		// print to terminal
		for (uint32_t i = 0; i < DEFAULT_BUFFER_LENGTH; i++) {
			char wav_str[10];
			sprintf(wav_str, "%d", (int)(read_buffer[i] >> 16));
			print(wav_str);
			print(",");
		}
